...
void check_one(char* buffer, int check) {
    ...
    if (buffer[check] == '\x6c') {
        ...
        //syslog(LOG_INFO, "[>] Check 1 failed\n");
...
void check_two(char* buffer, int check) {
    ...
    if (buffer[check] == '\x57') {
        ...
        //syslog(LOG_INFO, "[>] Check 2 failed\n");
        exit(-1);
    }
}
...
void check_three(char* buffer, int check) {
    ...
    if (buffer[check] == '\x21') {
        ...
        //syslog(LOG_INFO, "[>] Check 3 failed\n");
...
void vuln(char* buffer, size_t length) {
    ...
    syslog(LOG_INFO, "[>] vulnzzzzzzzzzz\n");
    char vulnBuff[20];
...
int main(int argc, char *argv[]) {
    ...
    syslog(LOG_INFO, "[>] runnnzzzzzzzzzzzzz\n");
