...
// globals /////////////////////////////////////// 
char* debugee = "vuln";                         //
size_t prototype_count = 7958;                  // size jpg
unsigned char input_prototype[7958];            //
unsigned char input_mutated[7958];              //
//void* fuzz_location = (void*)0x555555559000 + 0x188e;    //heap addr + find.py (Found 'JFIF' at 18a6) hexedit Cover.jpg dan hitung byte sampai JFIF yaitu 24 byte
void* fuzz_location = (void*)0x55555555b4a0;
int corpus_count = 0;                           //
unsigned char *corpus[100];                     //       
//////////////////////////////////////////////////

// breakpoints ///////////////////////////////////
long long unsigned start_addr = 0x0000555555555424; // alamat awal check_one
long long unsigned end_addr = 0x5555555551d0;   // alamat exit
//////////////////////////////////////////////////
...
struct dynamic_breakpoints {                    //
                                                //
    int bp_count;                               //
    long long unsigned bp_addresses[3];         //
    long long unsigned bp_original_values[3];   //
                                                //
};                                              //
...
void set_dynamic_breakpoints(pid_t child_pid) {
    ...
    vuln.bp_count = 3;
    vuln.bp_addresses[0] = 0x0000555555555692;
    vuln.bp_addresses[1] = 0x00005555555556a3;
    vuln.bp_addresses[2] = 0x00005555555556b6;
...
void log_corpus(unsigned char* crash_input, struct user_regs_struct crash_registers) {

    char file_name[0x30];
    sprintf(file_name, "corpus/11.%llx", crash_registers.rip);
...
void fuzzer(pid_t child_pid, unsigned char* snapshot_buf, struct user_regs_struct snapshot_registers, struct SNAPSHOT_MEMORY *read_memory) {
    ...
    while (1) {
        ...
        if (WIFSTOPPED(wait_status))
        {
            ...
            if (WSTOPSIG(wait_status) == 5)
            {
                ...
                if (temp_registers.rip - 1 != end_addr) { //jika alamat berbeda dengan exit, maka hit
                    //printf("%llx\n", temp_registers.rip-1);

                    add_code_coverage(child_pid, temp_registers);
                    log_corpus(input_mutated, temp_registers);
                }
            }
            else {
        ...
        restore_snapshot(snapshot_buf, child_pid, read_memory);
        ...
        iterations++;
        if (iterations % 500 == 0) { //update 0.5s
...
void execute_debugger(pid_t child_pid, struct SNAPSHOT_MEMORY *read_memory) {
    ...
    if (WIFSTOPPED(wait_status)) {
        if (WSTOPSIG(wait_status) == 5) {
            ...
            fuzz_location = (void*)registers.rax;
            printf("\033[1;35mdragonfly>\033[0m reached breakpoint at RIP: 0x%llx\n", (registers.rip - 1));
            printf("\033[1;35mdragonfly>\033[0m reached breakpoint at RAX: 0x%llx\n", (registers.rax));
        }
        else {
    ...
    unsigned char* snapshot_buf = create_snapshot(child_pid, read_memory);
    ...
    //system("clear");
    fuzzer(child_pid, snapshot_buf, snapshot_registers, read_memory);
}
...
int main(int argc, char* argv[]) {
    ...
    if (child_pid == 0) {
        ...
        execute_debugger(child_pid, &read_memory);
    }
